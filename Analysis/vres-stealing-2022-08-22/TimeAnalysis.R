...
# for (i in list(100, 500)) {
	...
	vt_label = function(l) {
	    ...
	    #lapply(l$vert, function(x) paste(if (x == "No symbiont") { "" } else { "Vertical transmission" }, x))
		x <- apply(l, 1, function(l) {
			if (l['vert'] == "No symbiont") { l['vert'] } else {
				paste0("VT ", l['vert'], ", HR ", l['ht_res'], ", VR ", l['vt_res'])
			}
		})
		# print(l)
		# print('-->')
	...
	task_data <- melt(initial_data, id.vars=c("update", "vert", "partner", "ht_res", "vt_res"), measure.vars=task_columns)
	for (t in task_columns) {
	    print(ggplot(data=subset(task_data, variable==t), aes(x=update, y=value, color=partner, fill=partner)) +
	    	...
	    	facet_wrap(c("vert", "ht_res", "vt_res"), labeller=vt_label)
	    	)
	}

	donated_data <- melt(subset(initial_data, partner=="symbiont"), id.vars=c("update","vert", "ht_res", "vt_res"), measure.vars=c("earned", "donated", "stolen"))
	...
	call_data <- melt(subset(initial_data, partner=="symbiont"), id.vars=c("update","vert", "ht_res", "vt_res"), measure.vars=c("donate_calls", "steal_calls"))
	...
	trans_data <- melt(subset(initial_data, partner=="symbiont"), id.vars=c("update","vert", "ht_res", "vt_res"), measure.vars=c("attempts_horiz", "success_horiz", "attempts_vert"))
	...
	trans_data <- melt(subset(initial_data, partner=="symbiont"), id.vars=c("update","vert", "ht_res", "vt_res"), measure.vars=c("success_horiz", "attempts_vert"))
	print(ggplot(data=trans_data, aes(x=update, y=value, color=variable, fill=variable)) +
	    stat_summary(fun.data="mean_se", geom=c("smooth"), se=TRUE) +
	    ylab("Transmission events") +
	    ggtitle("Symbiont transmission events") +
