...
folder = '../../Data/vres-stealing-2022-08-22/'

verts = ["NONE", 0.2, 0.8]
hrs = [100, 500]
vrs = [50, 100, 250]
reps = range(10,20)
header = "vert,ht_res,vt_res,rep,update,partner,donate_calls,donated,earned,steal_calls,stolen,mutualism,attempts_horiz,success_horiz,attempts_vert,task_NOT,task_NAND,task_AND,task_ORN,task_OR,task_ANDN,task_NOR,task_XOR,task_EQU"
...
for v in verts:
    for hr in hrs:
        for vr in vrs:
            uid = "VT_NONE" if v == "NONE" else f"VT_{v}_HR_{hr}_VR_{vr}"
            if uid == "VT_NONE":
                if done_none:
                    continue
                done_none = True
